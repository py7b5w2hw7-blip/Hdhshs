...
# ================== НОВЫЕ ТАРИФЫ ==================
PLANS = {
    "premium": {"label": "1 ТБ", "stars": 950, "crypto": "https://t.me/send?start=IVfBnFlf6v5b"},
    "full":    {"label": "50 ГБ", "stars": 600, "crypto": "https://t.me/send?start=IVfBnFlf6v5b"},
    "medium":  {"label": "15 ГБ", "stars": 450, "crypto": "https://t.me/send?start=IVCR8jU3BohU"},
    "small":   {"label": "5 ГБ",  "stars": 350, "crypto": None},
}
PLAN_NAMES = {"premium": "1 ТБ", "full": "50 ГБ", "medium": "15 ГБ", "small": "5 ГБ"}
...
def db_add_user(user: types.User, ref_by: int = None):
    ...
    cur = con.cursor()
    cur.execute("""
        INSERT OR IGNORE INTO users (user_id, username, first_name, joined_at, ref_by, banned, total_spent, last_daily, balance, ref_balance, ref_percent)
        VALUES (?, ?, ?, ?, ?, 0, 0, NULL, 0, 0, 20)
    """, (user.id, user.username, user.first_name,
          datetime.now().strftime("%Y-%m-%d %H:%M"), ref_by))
    con.commit()
    con.close()

def db_get_ref_balance(user_id: int) -> int:
    con = sqlite3.connect(DB)
    cur = con.cursor()
    row = cur.execute("SELECT ref_balance FROM users WHERE user_id=?", (user_id,)).fetchone()
    con.close()
...
def db_update_ref_balance(user_id: int, amount: int):
    con = sqlite3.connect(DB)
    con.execute("UPDATE users SET ref_balance = ref_balance + ? WHERE user_id=?", (amount, user_id))
    con.commit()
    con.close()
...
def db_add_purchase(user_id: int, plan: str, stars: int) -> tuple:
    ...
    if ref_by and ref_by != user_id:
        ...
        con.commit()
        con.close()
        db_update_ref_balance(ref_by, earned)
...
def db_get_ref_stats(user_id: int):
    ...
    balance = db_get_ref_balance(user_id)
    recent = cur.execute("""
...
def db_request_withdrawal(user_id: int, amount: int):
    ...
    current = db_get_ref_balance(user_id)
    if current < amount:
        return False
    db_update_ref_balance(user_id, -amount)
    con = sqlite3.connect(DB)
    cur = con.cursor()
    cur.execute("""
...
async def cmd_start(message: Message):
    ...
    con = sqlite3.connect(DB)
    cur = con.cursor()
    existing = cur.execute("SELECT 1 FROM users WHERE user_id=?", (user_id,)).fetchone()
    con.close()

    # Если пользователь новый — обрабатываем реферала
    if not existing:
        new_user = True
        if len(args) > 1 and args[1].startswith("ref"):
            try:
                ref_id = int(args[1][3:])
                if ref_id != user_id:
                    existing_ref = db_get_ref_by(user_id)
                    if existing_ref is None:
                        ref_by = ref_id
            except ValueError:
                pass

    # Если уже прошёл капчу — сразу меню
    if user_captcha.get(user_id, False):
    ...
    # Если пользователь новый — добавляем с рефералом
    if new_user:
        db_add_user(message.from_user, ref_by)
        # Отправляем уведомление рефереру (как в старом коде)
        if ref_by:
            if not db_has_ref_notify_sent(ref_by, user_id):
                db_mark_ref_notify_sent(ref_by, user_id)
                try:
                    invited_count = db_get_invited_count(ref_by)
                    level_name = get_level(invited_count + 1)
                    pct = db_get_ref_percent(ref_by)
    ...
    # Показываем капчу (если ещё не проходил)
    if not user_captcha.get(user_id, False):
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✔ Я не робот", callback_data="captcha_pass")]
        ])
        await message.answer(
            "👋 Добро пожаловать!\n\n"
            "Для продолжения подтверди, что ты человек 👇",
            reply_markup=kb
        )
    else:
        await message.answer(
...
@dp.callback_query(F.data == "captcha_pass")
async def captcha_pass(call: CallbackQuery):
    user_id = call.from_user.id
    user_captcha[user_id] = True
    
    await call.message.delete()
    await call.message.answer(
        "✅ Отлично! Ты человек.\n\nДобро пожаловать в TENDO! 🌿"
    )
    
    # Регистрируем пользователя в БД (если ещё нет)
    db_add_user(call.from_user)
...
async def show_profile(event):
    ...
    con = sqlite3.connect(DB)
    cur = con.cursor()
    row = cur.execute("SELECT balance, ref_balance, total_spent, joined_at, first_name, username, ref_percent FROM users WHERE user_id=?", (user_id,)).fetchone()
    con.close()
    ...
    balance, ref_balance, total_spent, joined_at, first_name, username, ref_percent = row
    ...
    text = (
        ...
        f"🤝 Реферальный баланс: <b>{ref_balance}⭐</b>\n"
...
async def ref_withdraw(call: CallbackQuery):
    ...
    # Защита от отрицательного баланса
    if not db_request_withdrawal(uid, s["balance"]):
        await call.answer("❌ Ошибка! Недостаточно средств.", show_alert=True)
        return
...
async def menu_stars(call: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="1 ТБ — 950 ⭐", callback_data="stars_premium"))
    kb.row(InlineKeyboardButton(text="50 ГБ — 600 ⭐", callback_data="stars_full"))
    kb.row(InlineKeyboardButton(text="15 ГБ — 450 ⭐", callback_data="stars_medium"))
...
async def menu_crypto(call: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    if PLANS["premium"]["crypto"]:
        kb.row(InlineKeyboardButton(text="1 ТБ — Оплатить", url=PLANS["premium"]["crypto"]))
